...
# PART A : Twitter API : Streaming API , Search API , User API
...
#####################################################################################################
#
# Here we initialze the count to 100 so that we can 100 tweets of the respective term
#
#####################################################################################################
...
#####################################################################################################
# 
# In this section we implement the process of getting 100 tweets of every Breaking Bad user given in
# user_names.txt file. We first create the csv writer object and open the csv so that the extracted
# tweet data can be written into it. Using twitter API the tweets are extract live using the screen_name.
# After this we write the extracted using writerow
#
#####################################################################################################    

    sn_file = open(user_name_file)
    url = "https://api.twitter.com/1.1/statuses/user_timeline.json"
    col_title=['user_name','tweet']
    w = csv.writer(sys.stdout,dialect='excel',delimiter=',')
    w.writerow(col_title)
    for un in sn_file:
        un=un.strip()
        parameters = [("screen_name", un),("count", 100)]
        response = twitterreq(url, "GET", parameters)
        tweet = json.loads(response.read().decode('utf‐8'))
        for t in tweet:
            w.writerow((un,t['text']))
